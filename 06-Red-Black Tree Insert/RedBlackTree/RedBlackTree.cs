...
public class RedBlackTree<T> : IBinarySearchTree<T> where T : IComparable
{
    ...
    private bool IsRed(Node node)
        => node != null && node.Color;

    private Node RotateRight(Node node)
    {
        // Switch parent & left child, preserving order
        var prevLeft = node.Left;
        node.Left = prevLeft.Right;
        prevLeft.Right = node;

        // Update colors
        prevLeft.Color = Black;
        node.Color = Red;

        // Update new child (prev parent) count
        node.Count = 1 + this.Count(node.Left) + this.Count(node.Right);
    ...
    {
        // Switch parent & right child, preserving order
        var prevRight = node.Right;
        node.Right = prevRight.Left;
        prevRight.Left = node;

        // Update colors
        prevRight.Color = Black;
        node.Color = Red;
